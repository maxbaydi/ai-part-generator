...
### Continue / Finish (продолжение или завершение)
Специальный режим для работы с существующим материалом. Вкладка `Continue / Finish` позволяет продолжить или завершить музыкальную фразу с учётом предшествующего контекста.

**Как использовать:**
1. Выберите MIDI-итем(ы), которые служат контекстом (обязательно).
2. Задайте Time Selection там, где нужно сгенерировать продолжение.
3. Перейдите на вкладку `Continue / Finish`.
4. Выберите настройки:
   - **Mode** (режим):
     - `Continue` — продолжить музыкальную идею, избегая финальных каденций и полного разрешения. Используйте, когда нужно развить тему.
     - `Finish` — завершить секцию с каденцией (V-I, IV-I, ii-V-I), разрешением на тонику и decrescendo. Финальная нота будет длинной и стабильной.
   - **Section Position** (позиция в секции):
     - `Start` — генерируемый фрагмент в начале секции, есть материал после. Создаётся вступление с развитием к существующему материалу.
     - `Middle` — генерируемый фрагмент в середине, есть материал до и после. Обеспечивается плавная связь с обеих сторон.
     - `End` — генерируемый фрагмент в конце секции, есть материал перед. Оптимально для завершения с каденцией.
5. Нажмите `CONTINUE` или `FINISH` (кнопка зависит от выбранного режима).

**Особенности:**
- AI анализирует мотив из выбранного материала (интервалы, ритм, характер) и сохраняет его в продолжении.
- Регистр и тональность берутся из исходного материала.
- При `Finish` AI использует каденциальные формулы, замедляет ритмическую активность и применяет decrescendo к концу.

**Примеры использования:**
- Есть 4 такта мелодии, нужно добавить ещё 4 такта развития → `Continue` + `Middle` или `End`.
- Нужно красиво завершить 8-тактовую фразу → `Finish` + `End`.
- Есть припев, нужно написать переход от куплета → `Continue` + `Start`.
...
- **Нужно продолжить существующую мелодию:** выделите MIDI-итем с мелодией, задайте Time Selection после него, используйте вкладку `Continue / Finish` с режимом `Continue`.
- **Нужно завершить музыкальную фразу:** выделите MIDI-итем, используйте вкладку `Continue / Finish` с режимом `Finish` и позицией `End`.
